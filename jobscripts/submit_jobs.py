...
for n in range(3):
    ...
    with open(fname, "w") as f:
        ...
        f.write("#PBS -l select=1:ncpus=2:mem=8gb\n")
        ...
        f.write(f"parallel --jobs 2 < /home/g.samarth/Woodard2013/job_scripts/ipjobs_ps_{n:02d}.sh\n")
    ...
    with open(fname, "w") as f:
        ...
        f.write("#PBS -l select=1:ncpus=8:mem=16gb\n")
        ...
        f.write("export TERM=xterm\n")
        f.write("echo $PBS_JOBID\n")
        f.write(f"parallel --jobs 8 < /home/g.samarth/qdPy/jobscripts/ipjobs_{n:02d}.sh\n")
        f.write("echo \"Finished at \"`date`\n")
    os.system(f"qsub {fname}")
